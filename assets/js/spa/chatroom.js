...
;
define("angular", ["lodash"], (function (global) {
    return function () {
        var ret, fn;
...
define('cs!common/module',['angular', 'cs!./namespaces', 'cs!dialogService/index', 'cs!./loadingContainer/index', 'angular-ui-router'], function(angular, namespaces, dialogService, loadingContainerModule) {
  ...
  module.controller('ActivateController', [
    '$scope', '$stateParams', '$http', "Auth", "$log", function($scope, $stateParams, $http, Auth, $log) {
      var apiURL, init;
      apiURL = '/api/v1';
      ...
      init = (function(_this) {
        return function() {
          if (($stateParams.apiKey != null) && ($stateParams.token != null)) {
            $scope.stateActivation = '';
            $scope.isBusy = true;
            return Auth.activate({
              ...
              return $scope.stateActivation = 'success';
            }, function(err) {
              $scope.errors = [err.data];
              return $scope.stateActivation = 'error';
            })["finally"](function() {
...
Module namespace configuration
 */
define('cs!chatroom/namespaces',[], function() {
  return {
    name: "chatroom",
    module: {
      name: "chatroom"
...
// Generated by CoffeeScript 1.7.1
define('cs!chatroom/controllers/MainControllerClass',['cs!./../namespaces'], function(namespaces) {
  var ClassController;
  ClassController = function($scope, EntityFactory, SocketEntityFactory, DialogService, $http, $sailsSocket, CurrentUser) {
    var currentUser;
    ...
    $scope.onDeleteUser = function(item) {
      ...
      return delDlg.result.then(function(res) {
        $scope.isBusy = true;
        return item.$delete().then(function(result) {
          $scope.isBusy = false;
    ...
    $scope.onShowHistory = $scope.onCreateEntity = function(_id) {
      if (_id == null) {
        _id = null;
      }
      return DialogService.create("templates/" + (namespaces.module.name.replace(/\.+/g, "/")) + "/form.entity.tpl.html", "Edit_" + (namespaces.module.name.replace(/\.+/g, "_")) + "_Controller", {
        id: _id
      }).result.then(function(data) {
        $scope.tableParams.reload();
        return console.log('data', data);
      }, function(err) {
        return console.log('eer', err);
      });
    };
    $scope.onSetCurrentChatRoom = function(item) {
      var _ref;
      if (((_ref = $scope.currentChatRoom) != null ? _ref.$isNewMsg : void 0) != null) {
        $scope.currentChatRoom.$isNewMsg = false;
      }
      $scope.currentChatRoom = item;
      $scope.currentChatMessages = item.msgs;
    };
    $scope.onSendMessage = function(_event, dailogId, message) {
      console.log('onSendMessage ', dailogId, message);
      $scope.isLocked = true;
      SocketEntityFactory.sendData(dailogId, message).then(function(data) {
        $scope.messagesForUser = '';
        return $scope.isLocked = false;
      }, function(err) {
        return $scope.isLocked = false;
      });
    };
    $scope.onStartDialog = function(_event, dailogId, message) {
      console.log('onStartDialog', dailogId);
      if ($scope.currentChatRoom != null) {
        $http.post("/api/v1/conversations/" + dailogId + "/operator", {
          id: dailogId,
          isactive: true,
          operator: currentUser().id
        }).then(function(data) {
          console.log(data);
          return $scope.currentChatRoom.operator = data.data.operator;
        });
      }
    };
    return $scope.onTerminateDialog = function(_event, dailogId, message) {
      console.log('onStartDialog', dailogId);
      if ($scope.currentChatRoom != null) {
        $http.put("/api/v1/conversations/" + dailogId, {
          isactive: false
        }).then(function(data) {
          $scope.currentChatRoom.isactive = false;
          $scope.currentChatRoom = null;
          return $scope.currentChatMessages = null;
        }, function(msg) {
 ...
 */
define('cs!chatroom/services/EntityFactoryClass',[], function() {
  var EntityClass, Entity_API_Url, RefEntity_N1_API_Url;
  Entity_API_Url = "/api/v1/messages";
  RefEntity_N1_API_Url = "/api/v1/fastanswers";
  EntityClass = function($resource, $http, ngTableParams) {
    var APIService, _tableParams;
    APIService = new $resource("" + Entity_API_Url + "/:chatroom/:action/:id", {
      id: "@id"
    }, {
      'query': {
        method: 'GET',
    ...
    _tableParams = new ngTableParams({
      ...
      getData: function($defer, params) {
        ...
        return APIService.query(paramSend, function(result, p) {
...
// Generated by CoffeeScript 1.7.1
define('cs!chatroom/services/SocketEntityFactoryClass',[], function() {
  
  var Entity_API_Url, SocketEntityFactoryClass;
  _.mixin({
  ...
  Entity_API_Url = "/api/v1/messages";
  SocketEntityFactoryClass = function($sailsSocket, $http) {
    var _conversationList, _dataList, _handlers, _loadMessages, _operatorsList, _sendMessage;
    _dataList = [];
    _handlers = {};
    _conversationList = [];
    _operatorsList = [];
    $http.get('/api/v1/operators').success(function(data) {
      return _.forEach(data, function(item) {
        return _operatorsList.push(item);
      });
    });
    $http.get('/api/v1/conversations').success(function(data) {
      return _.forEach(data, function(item) {
        return _conversationList.push(item);
      });
    });
    $sailsSocket.subscribe('messages', function(msg) {
      console.log('get on controller', msg);
      if (msg.verb === "create") {
        console.log('-=-', _conversationList);
        (_.find(_conversationList, {
          id: msg.data.dialog
        })).msgs.push(msg.data);
      } else {
    ...
    $sailsSocket.subscribe('conversations', function(msg) {
      console.log('conversations ', msg);
      switch (msg.verb) {
        case "create":
          console.log('event create conversations ', msg);
          msg.data.msgs = [];
          _conversationList.push(msg.data);
          break;
        case "update":
          console.log('event update conversations ', msg);
          _.extend(_.find(_conversationList, {
            id: msg.data.id
          }), msg.data);
      }
    });
    $sailsSocket.subscribe('operator', function(msg) {
      switch (msg.verb) {
        case "create":
        ...
        case "update":
          ...
          _.extend(_.find(_operatorsList, {
            id: msg.data.id
          }), msg.data);
      }
    });
    _loadMessages = function() {
      return $sailsSocket.get(Entity_API_Url).then(function(res) {
        _dataList.length = 0;
        angular.forEach(res.data.chatrooms, function(item) {
          item.msgs = _.sortBy(item.msgs, 'createdAt');
          return _dataList.push(item);
        });
        return _dataList;
      });
    };
    _sendMessage = function(dialogId, msg) {
      return $http.post(Entity_API_Url, {
        dialog: dialogId,
        body: msg
      });
    };
 ...
 */
define('cs!chatroom/controllers/EditEntityControllerClass',['cs!./../namespaces'], function(namespaces) {
  var ClassEditController;
  ClassEditController = function($scope, $modalInstance, EntityFactory, SocketEntityFactory, data) {
    $scope.editEntity = {};
    $scope.tableParams = EntityFactory.tableParams;
    ...
    $scope.onDeleteEntity = function(item) {
    ...
    /** confirm delete (used service) */
    return $scope.tableParams.filter()["id"] = data.id;
  };
...
(function(angular, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define('ng-table',['angular'], function(angular) {
...
define("sails-io", (function (global) {
    return function () {
        var ret, fn;
...
define("ngsails.io", ["sails-io","angular"], function(){});
...
module and class dependecies
 */
define('cs!chatroom/module',['angular', 'cs!./namespaces', 'cs!dialogService/index', 'cs!./controllers/MainControllerClass', 'cs!./services/EntityFactoryClass', 'cs!./services/SocketEntityFactoryClass', 'cs!./controllers/EditEntityControllerClass', 'ng-table', 'angular-resource', 'ngsails.io'], function(angular, namespaces, dialogService, MainControllerClass, EntityFactoryClass, SocketEntityFactoryClass, EditEntityControllerClass) {
  var module;
  ...
  module.config([
    "$stateProvider", "$urlRouterProvider", "AccessLevels", function($stateProvider, $urlRouterProvider, AccessLevels) {
      $stateProvider.state("user", {
        ...
        url: "/chatroom",
        templateUrl: "templates/" + (module.name.replace(/\.+/g, "/")) + "/index.tpl.html",
...
define('tpl!chatroom/templates/index.tpl.html', function() {return function(obj) { var __p=[],print=function(){__p.push.apply(__p,arguments);};with(obj||{}){__p.push('<!-- template "chatroom/templates/index.tpl.html"--> <!-- styles "chatroom.less"--> <!-- Red number means - initial message received, not handled by anyone.No color - operator sent unanswered message.Blue color - user replied operator (not initial message) that was not answere--><!--<style>    /*TODO: replace to css file*/    .initial_messages_received{        /*background-color: red;*/    }    .user_replied_operator{        /*background-color: blue;*/    }</style>--><section class="page_chatroom row-fluid"  ng-init="status = {open:true}; status2 = {open:true}; currentChatRoom = null;"         style="height: 100%;"         loading-container="isBusy || isLocked"         >      <div class="row" style="height: 100%;">         <article class="col-sm-4 affix"  style="height:100%">             <accordion  id="list_client" style="height:70%">                <accordion-group is-open="status.open">                    <accordion-heading>                         Conversations                        <i class="pull-right fa " ng-class="{\'fa-chevron-down\': status.open, \'fa-chevron-right\': !status.open}"></i>                     </accordion-heading>                    <ul class="list-group" id="clients_list">                         <li ng-repeat="item in conversationList track by $index" class="list-group-item"                             ng-if="item.isactive"                            ng-class="{\'active\':item.id == currentChatRoom.id,\'user_replied_operator\':item.isWaitAnswer && item.operator, \'initial_messages_received\':!!!item.operator.id }"                             ng-click="onSetCurrentChatRoom(item);">                            <span class="badge" ng-show="item.id == currentChatRoom.id">                                <i class="fa fa-exclamation"></i>                                </span>                              <span  ng-class="{\'text\':true, \'text-error\':item.id == currentChatRoom.id}">                                 <span  ng-if="!!item.operator.username">({{item.operator.username}})</span>                                 <span  ng-if="!!item.operator.email && !!!item.operator.username">({{item.operator.email}})</span>                                 {{item.client}}                            </span>                        </li>                      </ul>                 </accordion-group>             </accordion>             <accordion id="list_operator" style="height:30%">                <accordion-group is-open="status.open2">                    <accordion-heading>                         Operators online                        <i class="pull-right fa " ng-class="{\'fa-chevron-down\': status.open2, \'fa-chevron-right\': !status.open2}"></i>                     </accordion-heading>                    <!--                    <ul class="list-group" id="operator_list">                                            <li ng-repeat="item in operatorsList track by $index" ng-show="item.isLogin"class="list-group-item active"                                                 >{{item.username}}                                                                            <span class="badge " ng-show="item.isLogin">                                                                                <i class="fa fa-exclamation"></i>  </span>                                               </li>                                          </ul> -->                    <div class="list-group" id="operator_list">                        <a ng-repeat="item in operatorsList track by $index" ng-class="{\'active\':item.isLogin}" class="list-group-item"                            >{{item.username}}                         </a>                      </div>                 </accordion-group>             </accordion>         </article>        <article class="col-sm-offset-4 col-sm-8 overflowable" style="height: 100%;">            <div class="col-sm-12 panel panel-default" style="height: 100%; padding-bottom: 70px;"  >                  <h4 class="text text-left text-primary col-sm-12 panel-heading" ng-switch="">Message Window                     <strong ng-show="currentChatRoom.client" ng-switch="!currentChatRoom.operator"> from {{currentChatRoom.client}}                          <button ng-switch-when="false" ng-click="onTerminateDialog($event, currentChatRoom.id)" class="btn btn-xs btn-warning pull-right">                            <i class="fa fa-warning"></i> Terminate dialog                        </button>                        <button ng-switch-default ng-click="onStartDialog($event, currentChatRoom.id)" class="btn btn-xs btn-success pull-right">                            <i class="fa fa-wechat"></i> Start dialog                        </button>                    </strong>                </h4>                <!--                    <ul autoscroll-down class="list-group panel-body" id="chat_dialog">                                         <li ng-repeat="item in currentChatMessages  track by $index" class="list-group-item"                                             ngclass="{\'text-right\':item.owner == currentChatRoom.owner.id}">                                            <p  ng-class="{\'text\':true, \'text-warning\':item.From == currentChatRoom.From}">({{item.updatedAt|  date:" hh:mm:ss"}}) <span ng-if="item.operator">Operator {{item.operator.username}}: </span></p>                                            <span class="textMsg">                                                 {{item.Body}}                                              </span>                                        </li>                                    </ul> -->                <div autoscroll-down class="list-group panel-body" id="chat_dialog">                     <a ng-repeat="item in currentChatMessages  track by $index" class="list-group-item"                        ngclass="{\'text-right\':item.owner == currentChatRoom.owner.id}">                        <p  ng-class="{\'text\':true, \'text-warning\':item.From == currentChatRoom.From}">({{item.updatedAt|  date:" hh:mm:ss"}}) <span ng-if="item.operator">Operator {{item.operator.username}}: </span></p>                        <span class="textMsg">                             {{item.Body}}                          </span>                    </a>                </div>             </div>        </article>        <footer class="col-sm-offset-4 navbar-fixed-bottom" loading-container="isLocked">             <form class="form well-sm  col-sm-12" id="chatform" ng-submit="onSendMessage($event, currentChatRoom.id, messagesForUser)" >                 <div class="form-group">                    <div class="col-sm-10 col-lg-10">                        <textarea  ng-model="messagesForUser"                                   enter-submit="onSendMessage($event, currentChatRoom.id, messagesForUser)"                                   class="form-control col-sm-8"                                    id="messageText"                                     ng-disabled="!currentChatRoom || isLocked"                                     placeholder="Type message here..."                                   ></textarea>                    </div>                     <div class="col-sm-2 col-lg-2">                        <input type="submit" id="submit" class="btn btn-success col-sm-12" ng-disabled="!messagesForUser || !currentChatRoom || isLocked" value="Send"/>                    </div>            </form>        </footer>      </div> </section><!-- //template "index.tpl.html"-->');}return __p.join('');}});


define('tpl!chatroom/templates/form.entity.tpl.html', function() {return function(obj) { var __p=[],print=function(){__p.push.apply(__p,arguments);};with(obj||{}){__p.push('<!-- template "form.entity.tpl.html"--><!-- Modal --><style>    .modal-dialog{        width: 100%;        padding: 15px;    }</style><div class="modal-content">    <div class="modal-header">        <!--<h4 ng-if="!currEntity.id">Новый город поддержки</h4>-->        <h4 ng-if="currEntity.id">История переписки<strong>{{currEntity.name}}</strong></h4>    </div>    <form name="EditForm" class="form-horizontal" role="form" ng-submit="onSave(editEntity)" loading-container="isBusy" novalidate>        <div class="modal-body">            <table ng-table="tableParams" class="table table-striped table-fixed">                <tbody>                    <tr ng-repeat="item in $data">                        <td data-title="\'Пользователь\'">{{item.owner.username}}</td>                         <td data-title="\'Сообщение\'">{{item.text}}</td>                         <td data-title="\'Дата создания\'" sortable="\'createdAt\'">{{item.createdAt| date:\'short\'}}</td>                         <!--<td data-title="\'Дата обновления\'" sortable="\'updatedAt\'">{{item.updatedAt| date:\'short\'}}</td>-->                                                         <td data-title="\'Действия\'">                                                            <!--<button class="btn btn-xs btn-success" ng-click="onEditEntity(item.id)">Изменить</button>-->                                                            <button class="btn btn-xs btn-danger" ng-click="onDeleteEntity(item)">Удалить</button>                                                        </td>                    </tr>                </tbody>            </table>                   </div>        <div class="modal-footer">            <!--<button type="submit" class="btn btn-primary btn_save" ng-disabled="EditForm.$invalid">{{"DIALOGS_SAVE"| translate}}</button>-->            <button type="button" class="btn btn-default btn_cancel" ng-click="onCancel()">{{"DIALOGS_CLOSE"| translate}}</button>        </div>    </form></div><!-- //template " /form.entity.tpl.html"-->');}return __p.join('');}});
...
Cache content of template(s) with current name space
 */
define('cs!chatroom/templates/templatesCache',['cs!./../module', 'tpl!./index.tpl.html', 'tpl!./form.entity.tpl.html'], function(module, tplIndex, tplForm) {
  console.log("templates/" + (module.name.replace(/\.+/g, "/")) + "/index.tpl.html");
  return module.run([
    '$templateCache', function($templateCache) {
      $templateCache.put("templates/" + (module.name.replace(/\.+/g, "/")) + "/index.tpl.html", tplIndex({
        namespace: module.name.replace(/\.+/g, "_")
      }));
      $templateCache.put("templates/" + (module.name.replace(/\.+/g, "/")) + "/form.entity.tpl.html", tplForm({
        namespace: module.name.replace(/\.+/g, "_")
      }));
      $templateCache.put("templates/" + (module.name.replace(/\.+/g, "/")) + "/form.create.tpl.html", tplForm({
        namespace: module.name.replace(/\.+/g, "_")
      }));
      return $templateCache.put("templates/" + (module.name.replace(/\.+/g, "/")) + "/form.edit.tpl.html", tplForm({
        namespace: module.name.replace(/\.+/g, "_")
      }));
    }
  ]);
...
// Generated by CoffeeScript 1.7.1
define('cs!chatroom/index',['cs!./module', 'cs!./templates/templatesCache'], function(module) {
  module.run([
    function() {
      return console.log("init " + module['name']);
    }
...
define('page',['cs!./chatroom/index'],function(index){  
    return index; 
});
/*
...
define("angular-messages", ["angular"], function(){});
...
define("angularjs-toaster", ["angular","angular-animate"], function(){});

/**
 * bootstraps angular onto the window.document node
 * 
 */
define('../bootstrap_app',['domReady!', 'angular', 'core','page',  'angular-bootstrap','angular-ui-router','angular-animate', "angular-messages", 'angularjs-toaster', 'angular-sanitize','angular-resource'], function (document, angular, core, page) {
